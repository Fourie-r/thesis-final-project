...
export class ChatroomService {
  ...
  public createForeignChatroom: BehaviorSubject<string | null> = new BehaviorSubject(
    null
  );
  ...
  constructor(
    ...
    this.createForeignChatroom.pipe(
      switchMap(chatroomId => {
        first(),
        console.log(chatroomId);
        if (chatroomId) {
          this.authService.currentUser.subscribe(user => {
            this.db.collection(`chatrooms/${chatroomId}/chatrooms`).doc(this.authService.currentUserSnapshot.id).set({
              user: user,
              unread: false,
              owner: chatroomId
            }).then(() => {
            ...
            return this.db.doc(`chatrooms/${chatroomId}/chatrooms/${this.authService.currentUserSnapshot.id}`)
            .valueChanges();
          });
  ...
  public createMessage(msg: string): void {
    ...
    this.db.doc(`users/${chatroomId}`).valueChanges().pipe(first()).subscribe((user: User) => {
      if (user.currentChatroom !== this.authService.currentUserSnapshot.id) {
          this.db.doc(`chatrooms/${chatroomId}/chatrooms/${this.authService.currentUserSnapshot.id}`).valueChanges().pipe(first()).subscribe((chatroom) => {
  ...
  public setCurrentChatroom(id?: string): void{
    ...
    if (id) {
    this.db
    .doc(`chatrooms/${this.authService.currentUserSnapshot.id}/chatrooms/${id}`).update({unread: false})
    ...
    .catch(err => console.log('Error setting currentChatroom', err));
      ...
      this.db.doc(`users/${this.authService.currentUserSnapshot.id}`).update({currentChatroom: ' ' })
      .catch(err => console.log('Error setting currentChatroom', err));
